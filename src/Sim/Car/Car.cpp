...
void Car::_PreTickUpdate(float tickTime, SuspensionCollisionGrid *grid)
{
    ...
    { // Update steering
        ...
        {
            steerAngle +=
                (POWERSLIDE_STEER_ANGLE_FROM_SPEED_CURVE.GetOutput(absForwardSpeed_UU) - steerAngle) * _internalState.handbrakeVal;
        }
    ...
    { // At least 1 wheel is contacting, apply sticky forces
                ...
                if (!_internalState.hasDoubleJumped && !_internalState.hasFlipped && !_internalState.isAutoFlipping)
                {
                    ...
                    if (shouldFlip)
                    {
                        ...
                        // Replicated based on https://github.com/samuelpmish/RLUtilities/blob/develop/src/simulation/car.cc
                        {
                            ...
                            if (abs(dodgeDir.x()) < 0.1f)
                                dodgeDir.x() = 0;
                            if (abs(dodgeDir.y()) < 0.1f)
                                dodgeDir.y() = 0;
...
void Car::_BulletSetup(btDynamicsWorld *bulletWorld)
{
    { // Set up rigidbody and collision shapes
        ...
        btRigidBody::btRigidBodyConstructionInfo rbInfo = btRigidBody::btRigidBodyConstructionInfo(RLConst::CAR_MASS_BT, NULL, _compoundShape, localInertia);
